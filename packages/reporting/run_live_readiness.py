"""Run Phase 6.1 live readiness preflight."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Phase 6.1 live readiness preflight.")
    parser.add_argument("--observation-review-json", required=True, help="Phase 5.10 observation review JSON")
    parser.add_argument("--acceptance-json", required=True, help="Phase 5.10 acceptance JSON")
    parser.add_argument("--daily-report-json", required=True, help="Phase 6.1 daily report JSON")
    parser.add_argument("--risk-yml", required=True, help="Strategy risk.yml")
    parser.add_argument("--session-id", required=True, help="Preflight session id")
    parser.add_argument("--strategy-id", required=True, help="Strategy id")
    parser.add_argument("--allow-warnings", action="store_true")
    parser.add_argument("--min-observation-hours", default="2")
    ...
    environment = {
        ...
        "hummingbot_api_base_url_configured": bool(env_values.get("HUMMINGBOT_API_BASE_URL")),
        "alert_channel_configured": _alert_channel_configured(env_values),
    }
    report = build_live_readiness_report(
        observation_review=load_json(observation_path),
        acceptance_report=load_json(acceptance_path),
        ...
        thresholds=LiveReadinessThresholds(
            min_observation_hours=Decimal(args.min_observation_hours),
            max_initial_live_order_notional=Decimal(args.max_initial_live_order_notional),
        ),
        artifacts={
            "observation_review_json": str(observation_path),
            "acceptance_json": str(acceptance_path),
            "daily_report_json": str(daily_report_path),
            "risk_yml": str(risk_path),
        },
    )
    json_path = write_live_readiness_json(report, Path(args.output_json))
    md_path = write_live_readiness_markdown(report, Path(args.output_md))
    print(
        f"decision={report.decision} session={report.session_id} "
        f"output={json_path} markdown={md_path}"
    )
    if report.decision == "live_preflight_blocked":
        raise SystemExit(1)
