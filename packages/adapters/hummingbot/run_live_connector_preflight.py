"""Run Phase 6.3 live connector handoff and preflight."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Phase 6.3 live connector preflight.")
    parser.add_argument("--activation-checklist-json", required=True)
    ...
    parser.add_argument("--expected-connector", default="binance")
    parser.add_argument("--market-type", default="spot")
    parser.add_argument("--allowed-pair", action="append", required=True)
    parser.add_argument("--required-secret-field", action="append", default=[])
    parser.add_argument(
        "--env-file",
        action="append",
        default=[],
        help="Optional .env file to inspect",
    )
    ...
    activation_path = Path(args.activation_checklist_json)
    ...
    required_secret_fields = tuple(args.required_secret_field) or default_required_secret_fields(
        args.expected_connector
    )
    report = build_live_connector_preflight(
        activation_checklist=load_json(activation_path),
        ...
        required_secret_fields=required_secret_fields,
        artifacts={
            "activation_checklist_json": str(activation_path),
            "credential_allowlist_json": str(credential_path),
            "operator_signoff_json": str(signoff_path),
            "live_risk_yml": str(risk_path),
        },
    )
    json_path = write_live_connector_preflight_json(report, Path(args.output_json))
    md_path = write_live_connector_preflight_markdown(report, Path(args.output_md))
    print(
        f"decision={report.decision} session={report.session_id} "
        f"connector={report.expected_connector} output={json_path} markdown={md_path}"
    )
    if report.decision == "live_connector_preflight_blocked":
        raise SystemExit(1)
